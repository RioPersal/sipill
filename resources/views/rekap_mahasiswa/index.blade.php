          ...
          <table class="table table-bordered">
              <thead>
                  ...
                  <tr>
                    ...
                    <th class="text-center">CPL-11</th> --}}
                ...
                @endphp
                  <tr>
                    ...
                    @endforeach
                      ...
                      {{-- @foreach ($kelass->where('id_matkul', $matkul->id) as $kelas)
                        @if ($penilaian->where('id_kelas', $kelas->id)->where('id_mahasiswa',$mahasiswa->id)->count() == 0)
                          <td class="text-center align-middle bg-light" colspan="{{ $cepeel_count }}" style="white-space: nowrap;">
                            Data nilai tidak ditemukan. Silahkan input nilai terlebih dahulu!
                          </td>
                        @endif
                      @endforeach --}}
                    @endif
                  ...
                  <tr>
                    ...
                    {{-- @foreach ($cepeel as $footer_cpl)
                        ...
                        @foreach ($indikator->where('id_cpl', $footer_cpl->id) as $key => $footer_indikator)
                            @foreach ($matkuls as $mtkl)
                            @foreach ($kelass->where('id_matkul', $mtkl->id) as $kelas)
                              @foreach ($cpmk->where('id_indikator',$footer_indikator->id)->where('id_matkul',$mtkl->id) as $cpm)
                                @foreach ($bobot_cpmk->where('id_cpmk',$cpm->id) as $bobot)
                                  @if ($penilaian->where('id_mahasiswa',$mahasiswa->id)->where('id_kelas',$kelas->id)->where('id_asesmen',$bobot->id)->count() == 0)
                                    @php
                                      $jumlah_nilai_ind = 0;
                            ...
                            @foreach ($mat_ind->where('id_matkul', $mtkl->id)->where('id_indikator', $footer_indikator->id) as $matind2)
                                
                                @php
                                    $x = 100/$mtkl->sks;
                                    $s += $x*$matind2->bobot_indikator;
                                @endphp
                            @endforeach
                            @endforeach
                            @endforeach
                        ...
                        @endforeach
                        <th class="text-center">{{ $jumlah_nilai_ind }}</th>
                    @endforeach --}}
                  </tr>
              </tfoot>
          </table>
