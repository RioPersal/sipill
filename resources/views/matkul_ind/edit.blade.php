...
<div class="card">
  <div class="card-body">
    <form class="form-horizontal" form action="/pemetaan-matkul-dan-cpmk/edit/{matkulind:id}" method="POST">
        @csrf
            {{-- <input type="hidden" name="id" value="{{ old('id', $matkul_ind->id) }}"> --}}
                ...
                @foreach ($indikator->where('id_cpl', $cpl->id) as $ind)
                    <div class="form-group row">
                        ...
                        {{-- <div class="col-1 mt-2">
                            <input class="form-check-input ml-3" name="id_indikator[]" type="checkbox" value="{{ $ind->id }}" @foreach($matkul_ind as $matdin){{ old('id_indikator[]', $ind->id) == $matdin->id_indikator ? 'checked' : '' }}@endforeach>
                        </div> --}}
